...
export default function App() {
  ...
  return (
    <div className="container">
      <div className="row min-vh-100">
        <div className="col d-flex flex-column justify-content-enter align-items-center ">
          ...
          {(posts.length &&
            showingCreateNewPostForm === false &&
            postCurrentlyBeingUpdated === null) > 0 && renderPostsTable()}
  ...
  function onPostCreated(createPost) {
    ...
    alert(
      `Post successfully created.After Clicking OK,your new post titled '${createPost.title}' will show up in the table below.`
    );
    getPosts();
  }
  ...
  function onPostUpdated(updatedPost) {
    ...
    let postsCopy = [...posts];
    const index = postsCopy.findIndex((poststCopyPost, currentIndex) => {
      if (poststCopyPost.postId === updatedPost.postId) {
        return true;
      }
    });
    if (index !== -1) {
  ...
  function onPostDeleted(deletedPostPostId) {
    let postsCopy = [...posts];
    const index = postsCopy.findIndex((poststCopyPost, currentIndex) => {
      if (poststCopyPost.postId === deletedPostPostId) {
        return true;
      }
    });
    if (index !== -1) {
    ...
    setPosts(postsCopy);
    alert(
      `Post successfully deleted.After clicking OK,look at the table below to see your post dissapear.`
    );
  }
